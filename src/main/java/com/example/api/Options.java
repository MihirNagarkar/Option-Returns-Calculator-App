...
public class Options extends AppCompatActivity {
    ...
    private void fetchMidValue(String symbol) {
        String url = "https://api.marketdata.app/v1/stocks/quotes/" + symbol + "/?token=YOUR_API_TOKEN_HERE"; // Replace with your valid token
    ...
    private void calculateExpectedReturn() {
        ...
        if (midValue != null) {
            if (role.equals("Call Buy (CB)")) {
                // Calculate expected return for Call Buy
                expectedReturn = lotSize * (Math.max(0, midValue - strikePrice) - premium);
                isProfitable = (midValue - strikePrice) > premium;
            } else if (role.equals("Put Buy (PB)")) {
                // Calculate expected return for Put Buy
                expectedReturn = lotSize * (Math.max(0, strikePrice - midValue) - premium);
                isProfitable = (strikePrice - midValue) > premium;
            }

            // Display result with profit/loss indication
            String resultText = String.format("Expected Return: %.2f\n", expectedReturn);
            resultText += isProfitable ? "Profit scenario" : "Loss scenario";
            tvResult.setText(resultText);
        } else {
            Toast.makeText(this, "Mid value not available. Please fetch it first.", Toast.LENGTH_SHORT).show();
        }
    }
